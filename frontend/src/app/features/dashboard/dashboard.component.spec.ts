...
describe('DashboardComponent', () => {
  ...
  it('navigates to the selected worklist', () => {
    ...
    expect(router.navigate).toHaveBeenCalledWith(['/egresados'], { queryParams: { pendiente: 'sin-empresa' } });
  });

  it('navigates to sin-empleo worklist', () => {
    component.verPendiente('sin-empleo');
    expect(router.navigate).toHaveBeenCalledWith(['/egresados'], { queryParams: { pendiente: 'sin-empleo' } });
  });
  ...
  it('shows no pending message when counts are zero', () => {
    ...
    fixture.detectChanges();
    const noPending = fixture.nativeElement.querySelector('.empty-state');
    expect(noPending?.textContent).toContain('No hay pendientes');
  });
});
