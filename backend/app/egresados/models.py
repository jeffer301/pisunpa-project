...
class Programa(models.Model):
    ...
    def _normalize(name: str) -> str:
        stripped = unicodedata.normalize('NFD', name).encode('ascii', 'ignore').decode('utf-8')
        return stripped.strip().title()

    def clean(self):
        super().clean()
        canonical = self._normalize(self.nombre)
        if Programa.objects.filter(nombre__iexact=canonical).exclude(pk=self.pk).exists():
            raise ValidationError({'nombre': f'Ya existe un programa similar: "{canonical}".'})
        self.nombre = canonical
...
class ProfesorAsignatura(models.Model):
    ...
    profesor = models.ForeignKey(
        ...
        related_name='asignaciones_asignatura'
    )
